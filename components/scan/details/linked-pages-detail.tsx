          ...
          links.map((link) => (
            <div key={`${link.url}-${link.text}`} className="flex items-start gap-3 rounded-md border border-zinc-200 px-3 py-2 dark:border-zinc-800">
              ...
              <div className="min-w-0 flex-1">
                <p className="break-words text-sm font-semibold text-zinc-900 dark:text-zinc-100">{link.text}</p>
                <p className="break-all text-xs text-muted-foreground" title={link.url}>
                  {link.url}
                </p>
              </div>
            </div>
