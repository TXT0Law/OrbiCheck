...
import { useUrlGroupRunOperationalEvents } from "@/lib/hooks/use-operational-events";
...
export function GroupRunProgress({
  ...
  const eventsQuery = useUrlGroupRunOperationalEvents(run.groupId, run.id);
  ...
  return (
    <div className="space-y-3">
      ...
      <OperationalEventsCard
        title="Group Run Diagnostics"
        events={eventsQuery.data?.events ?? []}
        isLoading={eventsQuery.isLoading}
        error={eventsQuery.error}
        emptyMessage="No group run diagnostics recorded yet."
      />
    </div>
  );
}
