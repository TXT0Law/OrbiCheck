...
const genericErrorMsg = 'Request failed while processing this scan module.';
  ...
  const vercelHandler = async (request, response) => {
    ...
    try {
      ...
      if (error.message.includes('timed-out') || response.statusCode === 504) {
        errorCode = 408;
        response.status(errorCode).json({ error: timeoutErrorMsg });
        return;
      }
      response.status(errorCode).json({ error: genericErrorMsg });
    }
  };
  ...
  const netlifyHandler = async (event, context, callback) => {
    ...
    try {
      ...
      callback(null, {
        ...
        body: JSON.stringify({ error: isTimeout ? timeoutErrorMsg : genericErrorMsg }),
        headers,
      });
    }
  };
